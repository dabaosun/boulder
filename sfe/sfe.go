...
func (sfe *SelfServiceFrontEndImpl) Unpause(response http.ResponseWriter, request *http.Request) {
	...
	if request.Method == http.MethodGet && len(request.URL.Query()) <= 0 {
		...
		ok := sfe.stillFresh(unpauseParams.timestamp)
		if !ok {
		...
		ok = sfe.verifyHMAC(unpauseParams, wfeHMACBytes)
		if !ok {
			return
		}
...
type unpauseParams struct {
	...
	// RegistrationID of the to-be-unpaused account
	regID int64

	// Timestamp is a Unix timestamp supplied by the WFE when it
	// generated the unpause URL
	timestamp int64
}
...
func (sfe *SelfServiceFrontEndImpl) parseAndValidateUnpauseParams(input string) (*unpauseParams, error) {
	...
	fmt.Printf("parsedTimestamp: %v\n", parsedTimestamp)
	timestamp, err := time.Parse(time.UnixDate, parsedTimestamp)
...
// verifyHMAC takes an unpauseParams and bytes representing a HMAC-SHA256 hash
// generated by the WFE and compares it a newly computed HMAC using the SFEs
// unpause secret key. If a match is found, the Subscriber account can be
// unpaused, otherwise it will remain paused.
func (sfe *SelfServiceFrontEndImpl) verifyHMAC(upp *unpauseParams, wfeHMAC []byte) bool {
	if upp == nil {
		return false
	}

	hmac := hmac.New(sha256.New, []byte(sfe.unpauseKey))
	sfeData := upp.version + "." + fmt.Sprint(upp.regID) + "." + fmt.Sprint(upp.timestamp)
