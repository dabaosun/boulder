...
func (ra *RegistrationAuthorityImpl) checkNewOrderLimits(ctx context.Context, names []string, regID int64, isRenewal bool) error {
	...
	if newOrdersPerAccountLimits.Enabled() && (features.Get().CheckRenewalExemptionAtWFE && !isRenewal) {
		started := ra.clk.Now()
	...
	// TODO(#7511): Remove the feature flag check.
	if certNameLimits.Enabled() && (features.Get().CheckRenewalExemptionAtWFE && !isRenewal) {
		started := ra.clk.Now()
